...
check(
    "valid agent constructs",
    Agent(name="empirical", mandate="reason from data").name == "empirical",
)

print("\n[2] Panel size invariants")
expect_error(
    "two positions rejected (need >=3)",
    lambda: Record(
        id="1",
        created_at="t",
        question="q",
        tier=Tier.REVERSIBLE,
        positions=positions([Vote.YES, Vote.NO]),
        outcome=Outcome.MAJORITY,
    ),
)
expect_error(
    "four positions rejected (must be odd)",
    lambda: Record(
        id="1",
        created_at="t",
        question="q",
...
check(
    "Record has no synthesized-answer field",
    "synthesis" not in Record.model_fields
    and "answer" not in Record.model_fields
    and "final" not in Record.model_fields,
)

print("\n[4] Crux / outcome cross-field invariants")
expect_error(
    "SPLIT without crux rejected",
    lambda: Record(
        id="1",
        created_at="t",
        question="q",
...
expect_error(
    "FRAGILE_AGREEMENT without concurrence rejected",
    lambda: Record(
        id="1",
        created_at="t",
        question="q",
...
expect_error(
    "concurrence on non-fragile rejected",
    lambda: Record(
        id="1",
        created_at="t",
        question="q",
...
check(
    "minority is the single NO position, verbatim",
    len(maj.minority) == 1
    and maj.minority[0].vote is Vote.NO
...
async def _fake():
    fp = FakeProvider(
        rules=[("empirical", "YES because data"), ("principled", "NO because duty")],
        default="abstain",
    )
